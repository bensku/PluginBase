...
{
	public static void setupIntegration(InFamousPermissions plugin)
	{
		InFamousPermissionsVault perms = new InFamousPermissionsVault(plugin);
		plugin.getServer().getServicesManager().register(Permission.class, perms, plugin, ServicePriority.Highest);

		InFamousChatVault chat = new InFamousChatVault(plugin, perms);
