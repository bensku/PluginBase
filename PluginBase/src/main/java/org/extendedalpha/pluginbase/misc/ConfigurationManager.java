package org.extendedalpha.pluginbase.misc;
...
public class ConfigurationManager {
	...
	public void forceType(String key, String className) {
		try {
			...
			throw new EAUtilsException("Unknown type " + className);
		}
	}
	...
	public void insert(String key, Object def) {
		...
		if (config.contains(keyPrefixed)) {
			throw new EAUtilsException("Config item already exists: " + keyPrefixed);
		}
	...
	public Object get(String key) {
		...
		if (!config.contains(keyPrefixed)) {
			throw new EAUtilsException("No such config item: " + keyPrefixed);
		}
	...
	private void setItem(String key, String val) {
		...
		if (val == null) {
			...
			try {
				...
				throw new EAUtilsException("'" + val + "' is not a valid value for '" + key + "'");
			}
		} else {
			...
			try {
				...
				throw new EAUtilsException("Cannot convert '" + val + "' into a " + c.getName());
			} catch (IllegalArgumentException e) {
				e.printStackTrace();
				...
				if (e.getCause() instanceof NumberFormatException) {
					throw new EAUtilsException("Invalid numeric value: " + val);
				} else if (e.getCause() instanceof IllegalArgumentException) {
					throw new EAUtilsException("Invalid argument: " + val);
				} else {
					e.printStackTrace();
				}
			}
		...
		if (processedVal != null || val == null) {
			...
			throw new EAUtilsException("Don't know what to do with " + key + " = " + val);
		}
	}
	...
	private <T> void setItem(String key, List<T> list) {
		...
		if (config.getDefaults().get(keyPrefixed) == null) {
			throw new EAUtilsException("No such key '" + key + "'");
		}
		if (!(config.getDefaults().get(keyPrefixed) instanceof List<?>)) {
			throw new EAUtilsException("Key '" + key + "' does not accept a list of values");
		}
		if (listener != null && validate) {
