...
public class DeleteIslandChunk {
     ...
     * @param loc
     */
    public DeleteIslandChunk(Skyblock plugin, final Island island) {
	...
	for (int x = minChunk.getX(); x <= maxChunk.getX(); x++) {
		...
		if (regen) {
			...
			if (world.equals(Skyblock.getIslandWorld())) {
			    Skyblock.getNetherWorld().regenerateChunk(x, z);
			}
			if (world.equals(Skyblock.getNetherWorld())) {
			    Skyblock.getIslandWorld().regenerateChunk(x, z);
			}
		    }
		}
	    }
