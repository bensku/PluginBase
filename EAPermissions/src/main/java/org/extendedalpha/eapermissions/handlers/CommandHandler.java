...
import org.extendedalpha.core.chat.ComponentBuilder;
...
public class CommandHandler implements CommandExecutor
{
	...
	private List<EAPermissionsCommand> registeredPrefixedCommands;
	private List<EAPermissionsCommand> registeredCommands;

	private final EAPermissions plugin;

	public CommandHandler(EAPermissions plugin)
	{
		this.plugin = plugin;
	...
	/**
	 * Registers a non-prefixed {@link EAPermissionsCommand}.
	 *
	 * @param command Non-prefixed {@link EAPermissionsCommand} to register.
	 */
	public void registerCommand(EAPermissionsCommand command)
	{
	...
	/**
	 * Registers a prefixed {@link EAPermissionsCommand}. The commandPrefix
	 * must be set for this method to work.
	 *
	 * @param command Prefixed {@link EAPermissionsCommand} to register.
	 */
	public void registerPrefixedCommand(EAPermissionsCommand command)
	{
		Validate.notNull(command, "command cannot be null!");
	 ...
	 */
	public List<EAPermissionsCommand> getRegisteredCommands()
	{
	 ...
	 */
	public List<EAPermissionsCommand> getRegisteredPrefixedCommands()
	{
	 ...
	 *
	 * @param commandPrefix EAPermissionsCommand prefix
	 */
	public void setCommandPrefix(String commandPrefix)
	{
		Validate.notEmpty(commandPrefix, "prefix cannot be null or empty!");
		this.commandPrefix = commandPrefix;
		this.registeredPrefixedCommands = new ArrayList<EAPermissionsCommand>();
	...
	public final EAPermissionsCommand getCommand(String name)
	{
		Validate.notNull(name, "name cannot be null!");
		for (EAPermissionsCommand command : registeredPrefixedCommands)
		{
			if (name.equalsIgnoreCase(command.getName()) || command.getAliases().contains(name.toLowerCase()))
				return command;
		}

		for (EAPermissionsCommand command : registeredCommands)
		{
			if (name.equalsIgnoreCase(command.getName()) || command.getAliases().contains(name.toLowerCase()))
				return command;
		}
	...
	public boolean onCommand(CommandSender sender, org.bukkit.command.Command cmd, String label, String[] args)
	{
		if (args.length > 0)
		{
			...
			EAPermissionsCommand command = getCommand(name);
			if (command != null)
			{
	...
	private final EAPermissionsCommand getHelpCommand()
	{
