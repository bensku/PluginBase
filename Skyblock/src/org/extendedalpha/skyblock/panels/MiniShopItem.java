...
public class MiniShopItem {
	...
	try {
	    ...
	    if (price > 0D) {
		buyAndSell.add(Skyblock.getPlugin().myLocale().shopBuy + " " + quantity + " @ " + VaultHelper.econ.format(price));
	    }
	    if (sellPrice > 0D) {
		buyAndSell.add(Skyblock.getPlugin().myLocale().shopSell + " " + quantity + " @ " + VaultHelper.econ.format(sellPrice));
	    }
	    if (price < 0D && sellPrice < 0D) {
		buyAndSell.add(Skyblock.getPlugin().myLocale().shopOutOfStock);
	    }
		...
		if (!material.equals(Material.POTION)) {
			...
			// Skyblock.getPlugin().getLogger().info("Potion in shop is :"
	    ...
	    Skyblock.getPlugin().getLogger().severe("Problem parsing shop item from shop.yml so skipping it: " + material);
	    Skyblock.getPlugin().getLogger().severe("Error is : " + ex.getMessage());
	    ex.printStackTrace();
	    Skyblock.getPlugin().getLogger().info("Potential potion types are: ");
	    for (PotionType c : PotionType.values())
		Skyblock.getPlugin().getLogger().info(c.name());
	    Skyblock.getPlugin().getLogger().info("Potions can also be EXTENDED, SPLASH or EXTENDEDSPLASH, example WATER_BREATHING:EXTENDED");
	}
