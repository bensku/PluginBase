...
public class CmdSuffixReset extends InFamousPermissionsCommand
{
	public CmdSuffixReset(InFamousPermissions plugin)
	{
		super(plugin);
	...
	public void perform()
	{
		...
		else if (args.length == 1)
		{
			...
			sendpMessage("&6You have reset {0}''s suffix!", user.getName());

			Player target = user.getPlayer();
			if (target != null && ! hasArgument("--silent"))
				sendpMessage(target, "&6Your suffix has been reset!");
		}
	}
}