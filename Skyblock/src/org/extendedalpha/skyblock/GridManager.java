...
public class GridManager {
     ...
     * @param plugin
     */
    public GridManager(Skyblock plugin) {
	this.plugin = plugin;
	loadGrid();
    }
	...
	if (location == null || (!location.getWorld().equals(Skyblock.getIslandWorld()) && !location.getWorld().equals(Skyblock.getNetherWorld()))) {
	    return null;
	}
	...
	// In Skyblock, liquid may be unsafe
	...
	//plugin.getLogger().info("DEBUG: netherworld = " + Skyblock.getNetherWorld());
	return islandLocation.toVector().toLocation(Skyblock.getNetherWorld());
    }
	...
	for (Player player : plugin.getServer().getOnlinePlayers()) {
		...
		if (!player.getUniqueId().equals(uuid) && (plugin.getPlayers().hasIsland(player.getUniqueId()) || plugin.getPlayers().inTeam(player.getUniqueId()))) {
		    ...
		    if (spawn != null) {
			// go to island spawn
			player.teleport(Skyblock.getIslandWorld().getSpawnLocation());
	...
	Skyblock.getIslandWorld().setSpawnLocation(location.getBlockX(), location.getBlockY(), location.getBlockZ());
