...
import org.extendedalpha.skyblock.Island.Flags;
import org.extendedalpha.skyblock.SafeBoat;
...
public class IslandGuard implements Listener {
    ...
    public IslandGuard(final Skyblock plugin) {
	this.plugin = plugin;
	...
	if (loc.getWorld().equals(Skyblock.getIslandWorld())) {
	    return true;
	}
	if (Settings.createNether && Settings.newNether && loc.getWorld().equals(Skyblock.getNetherWorld())) {
	    return true;
	}
	return false;
    }
	...
	if (!e.getEntity().getWorld().equals(Skyblock.getIslandWorld())) {
	    return;
	}
	...
	if (!animal.getWorld().equals(Skyblock.getIslandWorld())) {
	    return;
	}
	...
	if (e.getEntity().getWorld().equals(Skyblock.getNetherWorld())) {
	    inNether = true;
	}
	...
	if (e.getClickedBlock() != null) {
		...
		if (!island.getIgsFlag(Flags.allowCropTrample)) {
		    e.getPlayer().sendMessage(ChatColor.RED + plugin.myLocale(e.getPlayer().getUniqueId()).islandProtected);
		    e.setCancelled(true);
		    return;
		}
	...
	if (e.getMaterial() != null) {
		...
		try {
			...
			boolean inNether = false;
			if (e.getPlayer().getWorld().equals(Skyblock.getNetherWorld())) {
			    inNether = true;
			}
			// Check spawn PVP
			if (island.isSpawn() && Settings.allowSpawnPVP) {
	...
	if (inWorld(player) || player.getWorld().equals(Skyblock.getNetherWorld())) {
	...
	if (!plugin.getGrid().playerIsOnIsland(e.getPlayer())) {
		...
		if (type == Material.EGG || type == Material.WHEAT || type == Material.CARROT_ITEM || type == Material.SEEDS) {
		    ...
		    if (island != null && (!island.getIgsFlag(Flags.allowBreeding) && !island.getMembers().contains(p.getUniqueId()))) {
			e.getPlayer().sendMessage(ChatColor.RED + plugin.myLocale(e.getPlayer().getUniqueId()).islandProtected);
			e.setCancelled(true);
			return;
		    }
