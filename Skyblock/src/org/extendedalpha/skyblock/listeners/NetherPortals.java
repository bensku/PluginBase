...
public class NetherPortals implements Listener {
    ...
    public NetherPortals(Skyblock plugin) {
	this.plugin = plugin;
    }
	...
	Location dest = event.getFrom().toVector().toLocation(Skyblock.getIslandWorld());
	if (event.getFrom().getWorld().getEnvironment().equals(Environment.NORMAL)) {
	    dest = event.getFrom().toVector().toLocation(Skyblock.getNetherWorld());
	}
	// Vehicles
	...
	if ((island == null && !Settings.allowPortalUse) || (island != null && !island.getIgsFlag(Island.Flags.allowPortalUse))) {
	...
	case NETHER_PORTAL:
	    ...
	    World homeWorld = Skyblock.getIslandWorld();
		...
		if (event.getFrom().getWorld().getEnvironment().equals(Environment.NORMAL)) {
			...
			event.setTo(Skyblock.getNetherWorld().getSpawnLocation());
			...
			event.setTo(Skyblock.getIslandWorld().getSpawnLocation());
			event.useTravelAgent(true); 
		    }
		}
	    } else {
		...
		//Location dest = island.getCenter().toVector().toLocation(Skyblock.getIslandWorld());
		Location dest = event.getFrom().toVector().toLocation(Skyblock.getIslandWorld());
		if (event.getFrom().getWorld().getEnvironment().equals(Environment.NORMAL)) {
		    // Going to Nether
		    dest = event.getFrom().toVector().toLocation(Skyblock.getNetherWorld());
		    // Check that there is a nether island there. Due to legacy reasons it may not exist
		    if (island.getCenter().toVector().toLocation(Skyblock.getNetherWorld()).getBlock().getType() != Material.BEDROCK) {
			...
			if (plugin.getGrid().bigScan(dest, 20) == null) {
			    ...
			    if (nether != null) {
				plugin.getIslandCmd().pasteSchematic(nether, island.getCenter().toVector().toLocation(Skyblock.getNetherWorld()), event.getPlayer());
			    } else {
		    ...
		    //plugin.getLogger().info("DEBUG: Teleporting to " + event.getFrom().toVector().toLocation(Skyblock.getNetherWorld()));
		}
		event.setCancelled(true);
	...
	// Check world
	if (!e.getLocation().getWorld().equals(Skyblock.getNetherWorld())) {
	    return;
	}
