...
import org.extendedalpha.pluginbase.subapi.chat.ClickEvent;
import org.extendedalpha.pluginbase.subapi.chat.ComponentBuilder;
import org.extendedalpha.pluginbase.subapi.chat.HoverEvent;
...
public abstract class Command implements CommandExecutor
{
	...
	public Command(SubPlugin plugin)
	{
		this.plugin = plugin;
	...
	public final void execute(CommandSender sender, String[] args)
	{
		...
		catch (Throwable ex)
		{
			...
			builder.append(FormatUtil.format("&c{0}", ex.toString())).event(new HoverEvent(HoverEvent.Action.SHOW_TEXT, stack.replace("\t", "    ")));
			sendMessage(builder.create());
		}
	}
	...
	protected final void invalidSyntax(String[] args)
	{
		...
		for (int i = 0; i < missing.size(); i++)
		{
			...
			String explanation = arg.getExplanation();
			if (explanation != null)
				builder.event(new HoverEvent(HoverEvent.Action.SHOW_TEXT, FormatUtil.format("&4{0}:\n&f{1}", arg.getArgument(), explanation)));
		}
