...
public class SimpleConfigManager {
	...
	Bukkit.getLogger().info("DEBUG:getConfigContent " + file.getName());
	if(!file.exists()) {
	    Bukkit.getLogger().info("DEBUG: file doesn't exist");
	    return null;
	}
	...
	try {
		...
		if(currentLine.trim().startsWith("#")) {
		    Bukkit.getLogger().info("DEBUG comment: " + currentLine);
		    addLine = currentLine.replaceFirst("#", pluginName + "_COMMENT_" + commentNum + ":\"");
		    whole.append(addLine + "\"\n");
		    //Bukkit.getLogger().info("DEBUG replaced with: " + addLine);
		    commentNum++;
    ...
    public InputStream getConfigContent(InputStream file) {
	...
	try {
		...
		if(currentLine.trim().startsWith("#")) {
		    ...
		    addLine = currentLine.replaceFirst("#", pluginName + "_COMMENT_" + commentNum + ":\"");
		    whole.append(addLine + "\"\n");
		    Bukkit.getLogger().info("DEBUG replaced with: " + whole);
		    commentNum++;
	...
	Bukkit.getLogger().info("Preparing to save");
	int lastLine = 0;
	int headerLine = 0;
	...
	for(String line : lines) {
	    Bukkit.getLogger().info("DEBUG prepare: " + line);
	    if(line.trim().startsWith(this.getPluginName() + "_COMMENT")) {
		String comment = line.substring(0,line.indexOf(this.getPluginName() + "_COMMENT")) 
