 ...
 *******************************************************************************/
package org.extendedalpha.skyblock.panels;
...
/**
 * @author ExileDev
 *         Provides a handy control panel and shop
 */
public class ControlPanel implements Listener {
     ...
     * @param plugin
     */
    public ControlPanel(Skyblock plugin) {
	this.plugin = plugin;
    ...
    /**
     * This loads the shop from the shop.yml file
     */
	...
	// Minishop
	store.clear();
	miniShopFile = Util.loadYamlFile("shop.yml");
	...
	if (items != null) {
	    ...
	    size -= (size % 9);
	    miniShop = Bukkit.createInventory(null, size, plugin.myLocale().islandShopTitle);
	    // Run through items
	    int slot = 0;
		...
		try {
		    ...
		    plugin.getLogger().warning("Problem loading shop item #" + slot);
	...
	if (miniShop != null && inventory.getName().equals(miniShop.getName())) { // The
		...
		if (clicked.equals(item.getItem())) {
			...
			if (item.getPrice() > 0D) {
				...
				message = (plugin.myLocale().shopYouCannotAfford).replace("[description]", item.getDescription());
			    } else {
				...
				if (r.transactionSuccess()) {
				    ...
				    message = plugin.myLocale().shopYouBought.replace("[number]", Integer.toString(item.getQuantity()));
				    ...
				    // + r.errorMessage;
				    message = (plugin.myLocale().shopBuyProblem).replace("[description]", item.getDescription());
				}
			    }
			}
			...
			if (player.getInventory().containsAtLeast(item.getItemClean(), item.getQuantity())) {
			    ...
			    message = plugin.myLocale().shopYouSold.replace("[number]", Integer.toString(item.getQuantity()));
			    message = message.replace("[description]", item.getDescription());
			    message = message.replace("[price]", VaultHelper.econ.format(item.getSellPrice()));
			} else {
			    // message =
			    // "You do not have enough of that item to sell it.";
			    message = (plugin.myLocale().shopSellProblem).replace("[description]", item.getDescription());
			    ;
			}
		    }
