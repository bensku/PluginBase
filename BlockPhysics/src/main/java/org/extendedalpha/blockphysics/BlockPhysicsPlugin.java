...
import org.extendedalpha.blockphysics.commands.*;
...
import org.extendedalpha.core.*;
import org.extendedalpha.core.commands.CommandManager;
...
public class BlockPhysicsPlugin extends JavaPlugin implements ConfigurationListener {
    ...
    public void onEnable() {
        ...
        MessagePager.setPageCmd("/blockphysics page [#|n|p]");
    ...
    public static BlockPhysicsPlugin getInstance() {
        return instance;
    }
        ...
        for (Flag<?> flag : DefaultFlag.getFlags()) {
            if (flag.getName().replace("-", "").equalsIgnoreCase(flagName2)) {
                if (flag instanceof StateFlag) {
                    ...
                    throw new EAUtilsException("Flag " + flagName + " is not a WorldGuard state flag");
                }
            }
        }
        throw new EAUtilsException("Unknown WorldGuard flag " + flagName);
    }
    ...
    public Object onConfigurationValidate(ConfigurationManager configurationManager, String key, Object oldVal, Object newVal) {
        if (key.startsWith("worldguard.") && !isWorldGuardAvailable()) {
            throw new EAUtilsException("WorldGuard plugin is not enabled");
        }

        if (newVal != null && pctPat.matcher(key).find()) {
            int pct = (Integer) newVal;
            EAValidate.isTrue(pct >= 0 && pct <= 100, "Value must be a percentage (0-100 inclusive)");
        } else if (key.equals("bracing_materials")) {
            //noinspection unchecked
            for (String s : (List<String>) newVal) {
                if (!s.startsWith("-") && Material.matchMaterial(s) == null) {
                    throw new EAUtilsException("Invalid material: " + s);
                }
            }
        } else if (key.equals("bracing_distance")) {
            EAValidate.isTrue((Integer) newVal >= 0, "Value must be >= 0");
        } else if (key.startsWith("transform.")) {
            String s = key.substring(key.indexOf('.') + 1);
            MaterialData from = BlockPhysicsPlugin.parseMaterialData(s);
            EAValidate.notNull(from, "Invalid material: " + s);
            MaterialData to = BlockPhysicsPlugin.parseMaterialData((String) newVal);
            EAValidate.notNull(to, "Invalid material: " + s);
            ...
            EAValidate.isTrue(v >= 1 && v <= 7, "Value must be in range 1-7 inclusive");
        }
        return newVal;
    }
    ...
    public boolean isOrphan(Block block) {
        for (BlockFace f : BlockPhysicsPlugin.allFaces) {
