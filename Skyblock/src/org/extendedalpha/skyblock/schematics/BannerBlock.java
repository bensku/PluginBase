...
public class BannerBlock {
	...
	patternKey.put("sh", PatternType.STRIPE_CENTER);
	...
	patternKey.put("sh", PatternType.STRIPE_MIDDLE);
	...
	try {
	    ...
	    // //Skyblock.getPlugin().getLogger().info("Base value = " +
	    // baseColor);
	    // baseColor green = 10
		...
		for (Tag pattern : patterns.getValue()) {
		    // Skyblock.getPlugin().getLogger().info("pattern = " +
		    // pattern);
			...
			// Skyblock.getPlugin().getLogger().info("mark = " +
			// mark.getValue());
			// Skyblock.getPlugin().getLogger().info("color = " +
			// markColor);
			DyeColor dColor = DyeColor.getByData(markColor.byteValue());
			// Skyblock.getPlugin().getLogger().info(" dye color = "
			// + dColor.toString());
