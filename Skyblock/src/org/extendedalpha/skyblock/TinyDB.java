...
public class TinyDB {
	...
	try {
		...
		if (count % 1000 == 0) {
		    System.out.println("[Skyblock]: Processed " + count + " names to database");
		}
		count++;
		try {
		    // Get UUID
		    ...
		    System.err.println("[Skyblock/AcidIsland]: Problem reading " + file.getName() + " skipping...");
		    //ex.printStackTrace();
		}
	    }
	    /** Map changes are not persisted yet, commit them (save to disk) */
	    recMan.commit();
	    System.out.println("[Skyblock]: Complete. Processed " + count + " names to database");
	    // Set flag
	    dbReady = true;
	} catch (Exception e) {
	    System.err.println("[Skyblock/AcidIsland] : Problem creating database");
	}
    }
